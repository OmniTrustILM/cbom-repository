...
This document decides how a consumer discovers new documents in the repository. It changes no code. Until the fields above are filled in, every "decided" wording in this document, in the follow-up issues filed from it and in the proposal comments on #26 and #27 is the author's recommendation, not a ratified decision; cbom-repository#138 stays open until the Tech Lead records the choice here (approval of PR #146 with the fields filled counts). Deliverable 1 of #138 (the `limit` parameter, warning entries, nested counting, the OpenAPI corrections) is on branch `feat/138-search-hardening-change-feed`, pending merge; this document describes it as specified there and builds on it.
...
- With `limit`: objects without `crypto-stats` metadata appear with `cryptoStats: null` and `warnings: ["crypto-stats-missing"]`; unparseable metadata gives `cryptoStats: null` and `warnings: ["crypto-stats-invalid"]`, and one bad object no longer fails the paged call. Without `limit` nothing changes: such objects are skipped or fail the call exactly as before #138. `GET /v1/bom/{urn}/versions` is unchanged as well (a version with missing statistics is skipped, unreadable statistics fail the call, no `warnings`).
- `CalculateCryptoStats` counts nested components (depth bound 1000, `components` tree only, matching Core's walker). New uploads carry the S3 user metadata `crypto-stats-version: 2`; its absence marks the legacy shallow count, and `crypto-stats-truncated: true` marks a count cut off by the 1000-level depth bound. Neither is on the JSON wire of the legacy call; the paged call surfaces them as the `crypto-stats-shallow` and `crypto-stats-truncated` warning codes. Warnings exist only in paged mode: the legacy call keeps its exact pre-#138 behaviour (metadata-less objects skipped, unreadable metadata fails the call), so a consumer that has not adopted `limit` sees no change at all.
...
| (iv) Missing or corrupt metadata | Objects without or with unparseable `crypto-stats` were invisible or broke the call. | Closed in paged mode (`limit`): visible with `warnings`. The unpaged call keeps its pre-#138 behaviour (skipped when missing, 500 when unreadable) so that consumers see no change until they adopt `limit`. | Core must tolerate `cryptoStats: null` (follow-up issue 2). Baseline: today the repository drops such objects, so Core never sees them; if Core dereferences the field after deliverable 1, the entry fails, is counted as skipped, and the moving watermark never shows it again, which is the same end state as today, not a regression. For invalid metadata paged mode is a strict improvement, because one bad object no longer fails the whole feed with 500. |
...
| (vi) Cost amplification | Every poll lists the whole bucket (`ceil(N / 1000)` LIST requests) regardless of how many objects changed, then issues one HEAD per candidate; paged mode repeats the full LIST for every page. | Bounded per call by `max(limit, entries in the boundary second)`, multiplied across pages; a dense second is unbounded until the keyset cursor (#144). | Options (b) and (c) address it; section 3.2 quantifies it. |
...
| Metadata versioning (`cryptoStats.version`) | On the #138 branch, pending merge, with a caveat | `crypto-stats-version: 2` is written to S3 metadata on new uploads and surfaces as the `crypto-stats-shallow` warning (paged mode) when absent. #27 section 8 asked for the version inside `cryptoStats` on the wire; deliverable 1 keeps the entry JSON byte-compatible and adds the information as a warning code instead. |
...
4. `crypto-stats-version` is S3 metadata; the paged feed surfaces its absence as the `crypto-stats-shallow` warning, the legacy feed does not. Until Core adopts `limit` (core#2208) it cannot tell a legacy-counted entry from a current one, and even then a warning is a hint, not a count. Treating every repository count as advisory and recounting all of them is the only consistent rule.
...
The drafts as filed are kept in [2026-09-02-change-feed-follow-up-issues.md](./2026-09-02-change-feed-follow-up-issues.md); in summary:
