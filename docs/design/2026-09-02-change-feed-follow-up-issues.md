...
The counts on the `cbom` table come from the repository's `cryptoStats` feed entry. cbom-repository#138 decided (section 11 of `docs/design/2026-09-02-change-feed-decision.md`) that the repository will not rewrite existing objects: objects uploaded before the counting fix keep a shallow count (nested components not counted), new uploads carry `crypto-stats-version: 2` in S3 metadata; the paged feed flags its absence as the `crypto-stats-shallow` warning, the legacy feed does not. Core therefore cannot tell a legacy count from a current one until it adopts `limit` (draft 2), and even then must treat every repository count as advisory.
