...
| `APP_HTTP_CORS_ALLOWED_ORIGINS` | ![](https://img.shields.io/badge/-NO-red.svg) | | comma-separated list of browser origins (`scheme://host[:port]`) allowed to read responses cross-origin, e.g. `https://ilm.example.com`. Empty disables CORS. Required when the ILM frontend health-checks this service, because that check runs in the operator's browser against the `cbomRepositoryUrl` platform setting. `*` allows any origin and is intended for development only — this service has no authentication of its own, so every listed origin gets full read access. |
